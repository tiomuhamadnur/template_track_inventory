        ...
        <div class="col-sm-12">
            <div class="home-tab">
                ...
                <div class="col-lg-12 grid-margin stretch-card mt-3">
                    <div class="card">
                        <div class="card-body">
                            <h4 class="card-title">Request For Inspection Depo</h4>
                                        ...
                                        @foreach ($data_rfi as $item)
                                            <tr>
                                                ...
                                                <td class="text-center text-wrap">
                                                    {{ $item->temuan_depo->line->code ?? '-' }} <br>
                                                    ({{ $item->temuan_depo->kilometer ?? '-' }} m)
                                                </td>
                                                <td class="text-center text-wrap">
                                                    {{ $item->temuan_depo->part->name ?? '-' }} <br>
                                                    {{ $item->temuan_depo->detail_part->name ?? '-' }} <br>
                                                    ({{ $item->temuan_depo->defect->name ?? 'Lainnya' }})
                                                </td>
                                                <td class="text-center">
                                                    {{ $item->tanggal ?? '-' }}
                                                </td>
                                                ...
                                                <td class="text-center">
                                                    <a href="#" data-bs-toggle="modal" data-bs-target="#ModalTemuan"
                                                        ...
                                                        data-photo="{{ asset('storage/' . $item->temuan_depo->photo) }}"
                                                        data-photo_close="{{ asset('storage/' . $item->temuan_depo->photo_close) }}">
                                                        Show
                                                    </a>
                                                </td>
                                                <td class="text-center">
            ...
            <div class="modal-content">
                <div class="modal-body p-2">
                    ...
                    <div class="px-5 pb-8 text-center mt-3">
                        <form action="{{ route('rfi.depo.delete') }}" method="POST">
                            @csrf
                            @method('delete')
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-body p-2">
                    ...
                    <div class="px-5 pb-8 text-center mt-3">
                        <form action="{{ route('rfi.depo.approve') }}" method="POST">
                            @csrf
                            @method('put')
            ...
            <div class="modal-content">
                ...
                <div class="modal-body p-2">
                    <div class="px-5 pb-8 mt-3">
                        <form action="{{ route('rfi.depo.update') }}" id="form_update_rfi" method="POST"
                            enctype='multipart/form-data'>
                            @csrf
                            @method('put')
                            <input type="text" name="id_rfi" id="id_rfi_modal" hidden>
                            <input type="text" name="id_temuan_depo" id="id_temuan_depo_modal" hidden>
                            <div class="col mb-3">
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#edit-rfi-modal').on('show.bs.modal', function(e) {
                ...
                var id_temuan_depo = $(e.relatedTarget).data('id_temuan_depo');
                $('#id_rfi_modal').val(id);
                $('#id_temuan_depo_modal').val(id_temuan_depo);
